...
/**
 * UniversityReviewSearch represents the model behind the search form of `common\models\UniversityReview`.
 */
class UniversityReviewSearch extends UniversityReview
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return [
            [['id', 'contact_number', 'university_name','rate_value_money', 'rate_infra', 'rate_food_acc', 'rate_college_crowd', 'rate_campus_life', 'rate_faculty', 'rate_visiting_faculty_lectures', 'rate_admin_staff', 'rate_course_curriculum', 'rate_internship', 'rate_placement'], 'integer'],
            [['visitor_name', 'university_name','highest_education', 'year_completion', 'current_status', 'email_id', 'sumerize_review', 'placement_percent', 'placement_salary_offer', 'placement_companies', 'placement_roles', 'placement_internship', 'infra_infrastructure', 'infra_falilities', 'hostel_boys', 'hostel_mesh', 'hostel_ac', 'hostel_other_facilities', 'hostel_bed_shared', 'hostel_girl_other_facilities', 'hostel_girl_mesh', 'hostel_girl_ac', 'hostel_girl_facilities', 'hostel_girl_bed_shared', 'facility_course', 'other_details_course_best', 'other_details_course_improve', 'other_details_course_extra', 'title_review', 'admission_recommend', 'other_reviews', 'review_status', 'course', 'program', 'created_at', 'updated_at'], 'safe'],
        ];
    }
    ...
    {
        $query = UniversityReview::find();//->joinWith(['program', 'course']);
        $query->leftJoin('program', 'program.id=university_review.program');        
        $query->leftJoin('courses', 'courses.id=university_review.course'); 
        
        if($id){
            $query->andWhere(['university_review.university_name'=>$id]);
        }
        
//        $query = UniversityReview::find();
        ...
        $query->andFilterWhere(['like', 'visitor_name', $this->visitor_name])
            ...
            ->andFilterWhere(['like', 'admission_recommend', $this->admission_recommend])
            ->andFilterWhere(['like', 'other_reviews', $this->other_reviews])
            ->andFilterWhere(['like', 'review_status', $this->review_status])
            ->andFilterWhere(['like', 'courses.name', $this->course])
